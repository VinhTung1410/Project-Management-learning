...
```mermaid
graph TD
    Project[Project Size & Complexity] --> Small[Small Team]
    Project --> Large[Large Team]

    Small --> Simple[Simple scope<br/>Few dependencies]
    Large --> Complex[Complex scope<br/>Multiple workstreams]
```
🧱 Instructor Rule:
Too small → overload
...
graph LR
    ...
    PM --> Team[Project Team]
    Customer --> PM
    User --> Product[Deliverables]
```
...
```mermaid
graph TD
    HighInfluence[High Influence] --> HighInterest[High Interest]
    HighInfluence --> LowInterest[Low Interest]
    LowInfluence[Low Influence] --> HighInterest
    LowInfluence --> LowInterest
```
...
graph TD
    ...
    A[Accountable<br/>Owns the outcome]
    C[Consulted<br/>Gives input]
    I[Informed<br/>Needs updates]
```
...
```mermaid
graph TD
    Failure[Project Failure] --> Expectations[Unclear Expectations]
    Failure --> Unrealistic[Unrealistic Commitments]
    Failure --> Communication[Poor Communication]
    Failure --> Resources[Lack of Resources]
    Failure --> Scope[Scope Creep]
```
### Common Causes
